...
if [ ! -d ${BZR_PO_DIR} ]; then
    echo "!Cloning bzr po repository (repo that Lauchnpad exports po updates to)."
    bzr branch lp:~jmehne/mnemosyne-proj/po-translations ${BZR_PO_DIR}
    if [ $? -ne 0 ]; then exitfunc "Could not clone po launchpad repo."; fi
fi
...
if [ ! -d ${BZR_POT_DIR} ]; then
    ...
    if [ $? -ne 0 ]; then exitfunc "Could not clone pot launchpad repo."; fi
fi
